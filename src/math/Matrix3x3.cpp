...
Matrix3x3::Matrix3x3(const std::vector<REAL>& values)
{
    ...
    memcpy(this->values, &values[0], 9 * sizeof(REAL));
}
...
Matrix3x3 Matrix3x3::inverse()
{
    ...
    REAL oneOverDeterminant = 1.0f / det;
...
const REAL& Matrix3x3::at(unsigned int column, unsigned int row) const
{
...
Matrix3x3 Matrix3x3::scaleMatrix(REAL v)
{
    Matrix3x3 m = identity;
    m.at(0, 0) = v;
    m.at(1, 1) = v;
...
Matrix3x3 Matrix3x3::rotationMatrix(REAL phi)
{
...
REAL Matrix3x3::determinantOfMinor(int x, int y)
{
    int r[2];
    for(int i = 0; i < 2; i++)
...
ettention::Vec3<REAL> multiply(const Matrix3x3& m, const ettention::Vec3<REAL>& v)
{
    const REAL x = m.at(0, 0)*v.x + m.at(1, 0)*v.y + m.at(2, 0)*v.z;
    const REAL y = m.at(0, 1)*v.x + m.at(1, 1)*v.y + m.at(2, 1)*v.z;
    const REAL z = m.at(0, 2)*v.x + m.at(1, 2)*v.y + m.at(2, 2)*v.z;
    return ettention::Vec3<REAL>(x, y, z);
}

ettention::Vec2<REAL> multiply(const Matrix3x3& m, const ettention::Vec2<REAL>& v)
{
    ettention::Vec3<REAL> v3(v.x, v.y, 1.0f);
    v3 = multiply(m, v3);
    return ettention::Vec2<REAL>(v3.x / v3.z, v3.y / v3.z);
}
