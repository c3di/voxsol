...
TEST_F(DiscreteProblemTests, BoundsHandling) {
    ...
    Material* outOfBounds = problem.getMaterial(libmmv::Vec3ui(3,3,3));
    EXPECT_TRUE(*outOfBounds == Material::EMPTY) << "Out of bounds coordinate did not return EMPTY material";

    try {
        problem.mapToVoxelIndex(libmmv::Vec3ui(3, 3, 3));
    ...
    unsigned int vertexIndex = problem.mapToVertexIndex(libmmv::Vec3ui(2, 2, 2));
    EXPECT_EQ(vertexIndex, 26) << "Expected 2x2x2 problem to return vertex index 26 at vertex coordinate (2,2,2)";

    try {
        problem.mapToVertexIndex(libmmv::Vec3ui(4, 3, 3));
...
TEST_F(DiscreteProblemTests, ProblemFragmentExtraction) {
    ...
    ProblemFragment fragment = problem.extractLocalProblem(libmmv::Vec3ui(1, 1, 1));
    ...
    ProblemFragment fragment2 = problem.extractLocalProblem(libmmv::Vec3ui(1,1,0));
...
TEST_F(DiscreteProblemTests, DirichletBoundaryPlacement) {
    ...
    problem.setDirichletBoundaryAtVertex(libmmv::Vec3ui(2, 2, 2), DirichletBoundary(DirichletBoundary::FIXED_Z));

    DirichletBoundary actual = problem.getDirichletBoundaryAtVertex(libmmv::Vec3ui(0, 0, 0));
...
TEST_F(DiscreteProblemTests, DirichletBoundaryApplication) {
    ...
    ProblemFragment fragment = problem.extractLocalProblem(libmmv::Vec3ui(1, 0, 0));
    ProblemFragment nonBoundaryFragment = problem.extractLocalProblem(libmmv::Vec3ui(2, 0, 0));
...
TEST_F(DiscreteProblemTests, NeumannBoundaryPlacement) {
    ...
    problem.setNeumannBoundaryAtVertex(0, NeumannBoundary(libmmv::Vec3<REAL>(1,2,3)));
    problem.setNeumannBoundaryAtVertex(libmmv::Vec3ui(2, 2, 2), NeumannBoundary(libmmv::Vec3<REAL>(6666, 7777, 8888)));

    NeumannBoundary actual = problem.getNeumannBoundaryAtVertex(libmmv::Vec3ui(0, 0, 0));
    EXPECT_TRUE(actual == NeumannBoundary(libmmv::Vec3<REAL>(1, 2, 3)));

    actual = problem.getNeumannBoundaryAtVertex(libmmv::Vec3ui(2, 2, 2));
    EXPECT_TRUE(actual == NeumannBoundary(libmmv::Vec3<REAL>(6666, 7777, 8888)));

    problem.setNeumannBoundaryAtVertex(0, NeumannBoundary(libmmv::Vec3<REAL>(0, 0, 0)));
    actual = problem.getNeumannBoundaryAtVertex(libmmv::Vec3ui(0, 0, 0));
    EXPECT_TRUE(actual == NeumannBoundary(libmmv::Vec3<REAL>(0, 0, 0)));

    actual = problem.getNeumannBoundaryAtVertex(libmmv::Vec3ui(1, 1, 1));
    EXPECT_TRUE(actual == NeumannBoundary(libmmv::Vec3<REAL>(0, 0, 0))) << "Expected vertex with no boundary condition to return a stress equal to (0,0,0)";
}
