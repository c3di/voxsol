...
TEST_F(MatrixPrecomputerTests, UniformSteel) {
    ...
    // Center node should not have a RHS matrix, should be identity
    const Matrix3x3* rhs = matStore.getRHS(13);
    EXPECT_TRUE(*rhs == Matrix3x3::identity);

    rhs = matStore.getRHS(1);
