...
TEST_F(CompressionLoadTest, SimpleCompression) {
    ...
    if (doOutputTestResultsAsVTK) {
        VTKSolutionVisualizer vis(problemInstance.getSolutionLOD(0));
        vis.writeToFile("d:\\tmp\\integration_SimpleCompression.vtk");
    }
...
TEST_F(CompressionLoadTest, SimpleCompressionAnisotropicVoxels) {
    ...
    if (doOutputTestResultsAsVTK) {
        VTKSolutionVisualizer vis(problemInstance.getSolutionLOD(0));
        vis.writeToFile("d:\\tmp\\integration_SimpleCompressionAnisotropicVoxels.vtk");
    }

}

TEST_F(CompressionLoadTest, SimpleCompressionNonUnitArea) {
    libmmv::Vec3ui discretization(100, 10, 10);
    ...
    REAL totalNeumannStressNewtons = asREAL(-1e9);
    bProjector.projectNeumannStressAlongNegX(totalNeumannStressNewtons);
    ...
    if (doOutputTestResultsAsVTK) {
        VTKSolutionVisualizer vis(problemInstance.getSolutionLOD(0));
        vis.writeToFile("d:\\tmp\\integration_SimpleCompressionNonUnitArea.vtk");
    }
